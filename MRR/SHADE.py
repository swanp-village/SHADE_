...
def mut_cross(MF_para_H, MCR_para_H, bounds, j, pop_size, obj_list_G, populations_G, P_i_int, Archive, dims, seed):
    ...
    # リスト同士として結合する（エラー修正）
    select_populations = list(Archive) + list(populations_G)
    
    Fi = -1.0
    while Fi <= 0.0:
        # size=1の配列が返るため、[0]をつけてスカラー（単一の値）にする
        Fi = stats.cauchy.rvs(loc=MF_para_H, scale=math.sqrt(0.1), size=1, random_state=rng)[0]
        if Fi > 1.0:
            Fi = 1.0
    ...
    xpbest = random.choice(xpbest_group)
    ...
    # Fiがスカラーになったため、mutatedはきれいな1次元配列になる
    mutated = populations_G[j] + Fi * (xpbest - populations_G[j]) + Fi * (a - b)
    
    # 境界処理（boundsを使って動的に計算）
    for i in range(dims):
        min_b = bounds[i][0]
        max_b = bounds[i][1]
        if mutated[i] < min_b:
            mutated[i] = (populations_G[j][i] + min_b) / 2
        elif mutated[i] > max_b:
            mutated[i] = (populations_G[j][i] + max_b) / 2

    trial = np.zeros(dims)
    # CRiも同様に[0]をつけてスカラーにする
    CRi = stats.norm.rvs(loc=MCR_para_H, scale=math.sqrt(0.1), size=1, random_state=rng)[0]
    if CRi > 1.0:
        CRi = 1.0
    elif CRi < 0.0:
        CRi = 0.0
    ...
    p = rng.random(dims)
    p[rng.choice([i for i in np.arange(len(p))], 1)] = 0
    
    # mutatedが1次元になったため、mutated[0][i] ではなく mutated[i] を使う
    for i in range(dims):
        if p[i] <= CRi:
            trial[i] = mutated[i]
        else:
...
def SHADE(func, bounds, params, pop_size, max_iter, H, tol, callback=None, rng=None):
    ...
    populations_G = populations.copy()
    obj_list = [func(pop, params) for pop in populations]
    obj_list_G = obj_list.copy()
    ...
    # アーカイブの初期化バグを修正（NumPy配列を使用）
    Archive = np.zeros((pop_size, xdim))
    Archivetimes = 0
    
    time_sta = time.perf_counter()
    
    # Poolの作成は世代ループの外側に配置（極めて重要な高速化）
    with Pool(processes=pop_size) as p:
        for i in range(max_iter):
            print("これは", i, "世代を表している")
            
            r = [random.randint(0, H-1) for _ in range(pop_size)]
            P_i = pop_size * random.uniform((2/pop_size), 0.2)
            ...
            mut_cross_paras = [
                [MF_para_H[r[j]], MCR_para_H[r[j]], bounds, j, pop_size, obj_list_G, populations_G, P_i_int, Archive, xdim, np.random.randint(0, 2 ** 32 -1)] 
            ...
            for j in range(pop_size):
                obj_list[j], populations[j], S_F, S_CR, delta_fk, Archive, Archivetimes = selection(
                    func, params, j, obj_list_G, populations_G, all_trial[j], all_Fi[j], all_CRi[j], S_F, S_CR, delta_fk, Archive, Archivetimes
                )
...
def selection(func, params, j, obj_list, populations, trial, Fi, CRi, S_F, S_CR, delta_fk, Archive, Archivetimes):
    ...
    #print(f"  [eval] j={j}, time={t1-t0:.4f}s, obj={obj_trial:.6f}")
...
def SHADE(func, bounds, params, pop_size, max_iter, H,  tol, callback=None, rng=None):
    ...
    if rng is None:
        print("[TRACE-02] rng is None -> create default_rng")
        rng = np.random.default_rng()
    ...
    lhs_samples = lhs(xdim, samples=pop_size, criterion="maximin")  # LHSで初期配置
    print(f"[TRACE-03] LHS done, shape={lhs_samples.shape}")
    populations = np.amin(dimbounds) + lhs_samples * (np.amax(dimbounds) - np.amin(dimbounds))


    populations_G = populations     #各世代Gの解を記録。世代毎のGを記録しておき、各解候補の更新は別のものに記録する。
    obj_list = [func(pop, params) for pop in populations]       #生成した初期解を関数に代入し評価値を返したリストを作成
    print(f"[TRACE-04] initial obj_list done, len={len(obj_list)}")
    obj_list_G = obj_list       #各世代Gの評価値を記録。扱いはpopulations_Gと同様
    best_x = populations[np.argmin(obj_list)]       #最もよい評価を得た際の解を記録,np.argminは最小の番号を返す
    best_obj = min(obj_list)        #最もよい評価を得た際の評価を記録する
    prev_obj = best_obj     #最もよい評価を今後比較のために記録する
    ...
    print(f"[TRACE-05] Archive[0] is Archive[1]: {Archive[0] is Archive[1]}")
    print("[TRACE-06] entering main loop")

    memory_update_taken = 0
    memory_update_skipped = 0
    improvement_taken = 0
    improvement_skipped = 0
    convergence_break_hit = False
    ...
    for i in range(max_iter):
        ...
        delta_fk = np.array([])
        if i < 2:
            print(f"[TRACE-08] gen={i} P_i_int={P_i_int}")
        ...
        tmp = list( p.map(wrapper_mut_cross, mut_cross_paras) ) #一時的な答え、この後スライスし、必要なところだけ切り取る
        #tmp = [wrapper_mut_cross(args) for args in mut_cross_paras]
        if i < 2:
            print(f"[TRACE-09] gen={i} Pool.map done, tmp_len={len(tmp)}")
            for I, t in enumerate(tmp):
                if t is None or len(t) != 3:
                    print(f"[TRACE-09-WARN] gen={i} worker={I} returned abnormal value: {t}")
        ...
        for I in range(pop_size):
            ...
            all_CRi[I] = tmp[I][2]
        if i < 2:
            print(f"[TRACE-10] gen={i} unpack done, all_Fi={all_Fi}")
            print(f"[TRACE-10] gen={i} all_CRi={all_CRi}")



        for j in range(pop_size):
            obj_list[j], populations[j], S_F, S_CR, delta_fk, Archive, Archivetimes = selection(func, params, j, obj_list_G, populations_G, all_trial[j], all_Fi[j], all_CRi[j], S_F, S_CR, delta_fk, Archive, Archivetimes)
        print(f"[TRACE-11] gen={i} selection loop done, S_F.size={S_F.size}, improved_count={S_F.size}")

        if S_F.size !=0 and S_CR.size !=0:
            print(f"[TRACE-12-TAKEN] gen={i} MEMORY UPDATE BRANCH TAKEN, k={k}")
            memory_update_taken += 1
            #MF_para_H[k] = ( sum( ( delta_fk * (S_F ** S_F) ) / sum(delta_fk) ) ) / ( sum( ( delta_fk * S_F ) / sum(delta_fk) ) )
            MF_para_H[k] = np.average(S_F * S_F, weights = delta_fk) / np.average(S_F, weights = delta_fk)
            MCR_para_H[k] = np.average(S_CR, weights = delta_fk)
            k = k + 1
            ...
            print(f"[TRACE-12-SKIP] gen={i} MEMORY UPDATE SKIPPED (S_F empty)")
            memory_update_skipped += 1
        #print("記録メモリ F = ",MF_para_H)
        #print("記録メモリ CR = ",MCR_para_H)
        #print("scipy Fi = ",all_Fi)
        #print("scipy CRi = ",all_CRi)
        print(f"[DIVERSITY] gen={i}, best_obj={best_obj:.4f}, std={np.std(obj_list):.6f}, mean={np.mean(obj_list):.6f}, pop_mean_per_dim={np.mean(populations, axis=0)}, pop_std_per_dim={np.std(populations, axis=0)}")
        ...
        if best_obj < prev_obj:     #一周ごとに更新後の最高評価と更新前の最高評価を比べる
            print(f"[TRACE-13-TAKEN] gen={i} IMPROVEMENT BRANCH TAKEN (best_obj={best_obj:.4f} < prev_obj={prev_obj:.4f})")
            improvement_taken += 1
            #print("std(標準偏差？) = ", np.std(obj_list))
            #print("mean(平均) = ", np.mean(obj_list))
            #print("評価値確認",obj_list)
    ...
    print("[TRACE-15] SHADE end, returning")
    print(f"[TRACE-SUMMARY] memory_update_taken={memory_update_taken}, memory_update_skipped={memory_update_skipped}")
    print(f"[TRACE-SUMMARY] improvement_taken={improvement_taken}, improvement_skipped={improvement_skipped}")
    print(f"[TRACE-SUMMARY] convergence_break_hit={convergence_break_hit}")
    return best_x, best_obj     #best_x➡最適化が終わったK, best_obj➡最適化が終わったE
...
def mut_cross(MF_para_H, MCR_para_H, bounds, j, pop_size, obj_list_G, populations_G, P_i_int, Archive, dims, seed):
    ...
    while Fi <= 0.0:
        ...
        fi_retry += 1
        if fi_retry > 50:
            print(f"[TRACE-MC-WARN] j={j} Fi sampling retried {fi_retry} times, MF_para_H={MF_para_H}")
            break
    #Fi = 0.5
    A = np.array(obj_list_G)
    A_sort_index = np.argsort(A)        #ここ二行でobj_list_Gのソートを行っている。評価の良い順に並べ、そのインデックスがリストになっている。
    xpbest_group = [populations_G[A_sort_index[i]] for i in range(P_i_int)]  
    if j == 0:
        print(f"[TRACE-MC-03] P_i_int={P_i_int}, xpbest_group_size={len(xpbest_group)}")#G世代の解候補の中から、評価が高いものをP_i_intの数だけ選んだ集合を作る。
    xpbest = random.choice(xpbest_group)        #G世代の解候補の中から上位N×P番目までの候補から一つを選んだ。
    indexes = [i for i in range(pop_size) if i != j]        #jは現在選んでいる解。それ以外の番号を指定しているインデックスを作成
    a = populations_G[rng.choice(indexes, 1, replace = False)]        #現在選んでいる解以外から1つを選ぶ。
    b = random.choice(select_populations)       #アーカイブも含めて解候補の中から解を一つ選ぶ。➡「要改変」アーカイブは問題ないが、G世代の解候補から選ぶ際に、jを除いていない。これにより注目しているものと同じ要素を選ぶ可能性がある。
    ...
    while np.all(b == 0.0):           #bが0のときは一生新たに選択をする。bがゼロとなるのは埋まっていないアーカイブを選択した場合である。
        ...
        zero_retry += 1
        if zero_retry > 100:
            print(f"[TRACE-MC-WARN] j={j} b==0.0 retried {zero_retry} times (Archive still mostly empty?)")
            break
    mutated = populations_G[j] + Fi * (xpbest - populations_G[j]) + Fi * (a - b)       #突然変異を表す式。「要改変」➡現在はカレントトゥベスト➡最終的にはカレントトゥピーベストにする
    #変異によって生まれたベクトルが異常な場合、値を範囲内に収める。
    correction_count = 0
    for i in range(dims):
        if mutated[0][i] <= 0:
            ...
            mutated[0][i] = (populations_G[j][i] + 0.996) / 2
            correction_count += 1
    if j == 0 and random.random() < 0.01:
        print(f"[TRACE-MC-02] j={j} boundary_correction_count={correction_count}/{dims}")
